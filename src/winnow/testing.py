...
- :func:`assert_golden` — snapshot testing. A connector ships one golden
  snapshot per behaviour; ``WINNOW_ACCEPT=1`` (re)writes snapshots, CI runs
...
def _golden_dir() -> Path:
    return Path(os.environ.get("WINNOW_GOLDEN_DIR", str(DEFAULT_GOLDEN_DIR)))
...
def assert_golden(name: str, actual: object) -> None:
    ...
    With ``WINNOW_ACCEPT=1`` the snapshot is (re)written instead of compared.
    ...
    if os.environ.get("WINNOW_ACCEPT"):
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(payload, encoding="utf-8")
        return
    if not target.exists():
        raise AssertionError(
            f"golden snapshot missing: {target} — run with WINNOW_ACCEPT=1 to create it"
        )
    if target.read_text(encoding="utf-8") != payload:
        raise AssertionError(
            f"golden drift in {target}: output changed — review, then WINNOW_ACCEPT=1"
        )
